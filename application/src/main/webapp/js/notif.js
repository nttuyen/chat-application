...
ChatNotification.prototype.initUserProfile = function(callback) {
  ...
  jqchat.ajax({
    ...
    success: function(data){
      ...
      this.refreshNotif();
      chatApplication.setNotificationCallback(jqchat.proxy(this.notificationCallback, this));
...
ChatNotification.prototype.notificationCallback = function(data) {
  if (data === false) {
    var $chatNotification = jqchat("#chat-notification");
    $chatNotification.html('<span></span>');
    $chatNotification.css('display', 'none');
  ...
  } else if(this.oldNotifTotal!=data.notifications.length){
    var notifyMe = false;
    data.notifications.sort(function(el1, el2){
      return el2.timestamp - el1.timestamp;
    });
